...
auth('15801806963', 'Royalflush0000')  # 账号是申请时所填写的手机号
#上海证券交易所  .XSHG
#深圳证券交易所  .XSHE
...
def get_single_price(code, count, time_freq, start_date=None, end_date=None):
    """
    获取单个股票行情数据
    :param code: '000001.XSHE'
    :param count: 100
    :param time_freq: 'daily'
    :param start_date: '2021-02-22'
    :param end_date: '2022-02-22'
    :return: 
    """
    ...
    # 获取行情数据
    data = get_price(code, count=count, start_date=start_date, end_date=end_date,
...
def transfer_price_freq(data, time_freq):
    ...
    df_trans['volumn(sum)'] = data['volume'].resample('W').sum()
    df_trans['money(sum)'] = data['money'].resample('W').sum()
...
def get_single_finance(code, date, statDate):
    ...
    #存储财务指标
    data.to_csv('/Users/yangjie/PycharmProjects/data/finance/finance2020.csv')
    #基于盈利指标选股：eps,operating_profit,roe,inc_net_profit_year_on_year
    data = data[(data['eps']>0) & (data['operating_profit'] > 2212173617) & (data['roe'] > 11 )
                & (data['inc_net_profit_year_on_year'] > 10)]
    return data
